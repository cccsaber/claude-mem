...
function buildMcpNodeLauncher(options: ShellTemplateOptions): string {
  ...
  return (
    ...
    `if(!env.CLAUDE_MEM_PLATFORM_SOURCE&&R.includes(p.join('.codex','plugins','cache')))env.CLAUDE_MEM_PLATFORM_SOURCE='codex';` +
    `const ch=c.spawn(process.execPath,[p.join(R,'scripts',${require})],{stdio:'inherit',env});` +
