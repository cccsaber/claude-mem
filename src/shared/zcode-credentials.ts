...
export function readZcodeApiKey(): ZcodeCredentialResult {
  const legacyOverride =
    process.env.CLAUDE_MEM_ZCODE_CREDENTIALS_PATH ||
    process.env.CLAUDE_MEM_ZCODE_CREDENTIALS_DIR;

  // Priority 1: v2/config.json (current/active key), unless the caller
  // explicitly points us at a legacy credentials file/dir for tests or
  // advanced setups.
  if (!legacyOverride) {
